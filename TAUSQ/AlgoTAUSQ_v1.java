...
public class AlgoTAUSQ {
    ...
    public void runAlgorithm(String input, String output, double utilityratio) throws IOException {
        ...
        Map<Integer, Map<Integer, Integer>> mapRsIUtility = new HashMap<>();
        ...
        try {
            ...
            while ((thisLine = myInput.readLine()) != null) {
                ...
                Map<Integer, Map<Integer, Integer>> LocalmapRsIUtility = new HashMap<>();
                ...
                Map<Integer, Integer> mapRsSIU = new HashMap<>();
                                ...
                                for (int k = i + 1; k < itemBufferLength; k++) {
                                    ...
                                    if (tempItem != -1) {
                                        if (mapRsSIU.get(tempItem) == null) {
                                            mapRsSIU.put(tempItem, tempUtility);
                                        } else {
                                            int lastSIU = mapRsSIU.get(tempItem);
                                            if (lastSIU < tempUtility) {
                                                mapRsSIU.put(tempItem, tempUtility);
                                            }
                                        }
                                    }
                ...
                for (Entry<Integer, Map<Integer, Integer>> entry : LocalmapRsIUtility.entrySet()) {
                    int item = entry.getKey();
                    // 检查 mapRsIUtility 中是否已有对应的 item
                    if (mapRsIUtility.get(item) == null) {
                        // 如果没有，则直接放入
                        mapRsIUtility.put(item, new HashMap<>(entry.getValue())); // 深拷贝
                    } else {
                        // 如果已有，则合并
                        for (Map.Entry<Integer, Integer> entry1 : entry.getValue().entrySet()) {
                            mapRsIUtility.get(item).merge(entry1.getKey(), entry1.getValue(), Integer::sum);
                        }
                    }
                }
                ...
                for (Entry<Integer,ArrayList<TargetedList>> entry: mapItemUC.entrySet()){
                    ...
                    for(int i=0;i<entry.getValue().size();i++){
                        ...
                        for(int j=0;j<entry.getValue().get(i).LengthOfUtilityList;j++){
                            ...
                            System.out.println("  rrl:"+entry.getValue().get(i).List.get(j).rrl);
                        }
        ...
        for(Entry<Integer, Integer> entry : mapItemSRAU.entrySet()){
            ...
            for (Map.Entry<Integer, Integer> entry1 : mapRsIUtility.get(item).entrySet()) {
            	int tempItem = entry1.getKey();//定义局部变量，避免重复访问类运算
            	int tempValue = entry1.getValue();
               if (tempValue < minAU) {
            	   if (!tempSufSet.contains(tempItem)) {
                	tempLowIUtility += tempValue;
        ...
        if(DEBUG==5){
            ...
            for (int i = 0; i < uc.size(); i++) {
                ...
                for (int j = 0; j < uc.get(i).LengthOfUtilityList; j++) {
                    ...
                    System.out.println("  rrl:" + uc.get(i).List.get(j).rrl);
                }
                System.out.println("End of a UtilityList");
            }
            System.out.println("#####################");
